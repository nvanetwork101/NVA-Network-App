...
function RoastRoomContent({ battleState, currentUser, creatorProfile, showMessage, handleExit }) {
    ...
    const [faceCoords, setFaceCoords] = useState({ x: 50, y: 30 }); 
    const [flyingEmojis, setFlyingEmojis] = useState([]);

    useEffect(() => {
        const handleFaceData = (e) => { if (e.detail) setFaceCoords(e.detail); };
        window.addEventListener('faceTrackingData', handleFaceData);
        return () => window.removeEventListener('faceTrackingData', handleFaceData);
    }, []);
    ...
    return (
        <div className={`${isFinalSeconds ? 'shake-active' : ''} ${isSuspense ? 'hud-warning-border' : ''}`} style={{ height: '100%', width: '100%', position: 'relative', display: 'flex', flexDirection: 'column', backgroundColor: '#050505', overflow: 'hidden', transition: 'box-shadow 0.3s ease' }}>
            ...
            <div style={{ position: 'absolute', top: '70px', left: '50%', transform: 'translateX(-50%)', zIndex: 100, pointerEvents: 'none' }}>
                <div className="glass-pill" style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '8px 24px', border: `1px solid ${battleState.hostStreak >= 0 ? 'rgba(0,255,255,0.2)' : 'rgba(255,42,42,0.2)'}` }}>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                        <span style={{ color: '#AAA', fontSize: '9px', fontWeight: '800', letterSpacing: '0.1em', textTransform: 'uppercase' }}>Host Streak</span>
            ...
            <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <div className="video-grid" style={{ gridTemplateColumns: tracks.length > 1 ? '1fr 1fr' : '1fr', gridTemplateRows: tracks.length > 1 && window.innerWidth < 768 ? '1fr 1fr' : '1fr' }}>
                    {tracks.length > 0 ? tracks.map((t, idx) => {
                        const isPrimaryTarget = true; 
                        ...
                        return (
                            <div key={`${t.participant.identity}-${t.source}`} className="video-cell">
                                ...
                                {/* EMOJI ANCHOR */}
                                {isPrimaryTarget && (
                                    <div style={{ position: 'absolute', top: `${faceCoords.y}%`, left: `${faceCoords.x}%`, transform: 'translate(-50%, -50%)', zIndex: 50, pointerEvents: 'none' }}>
                                        {flyingEmojis.map(e => (
                                            <span key={e.id} style={{ position: 'absolute', fontSize: '48px', left: '-24px', top: '-24px', animation: 'fly-up 1s cubic-bezier(0.25, 1, 0.5, 1) forwards', filter: 'drop-shadow(0 10px 15px rgba(0,0,0,0.5))' }}>
                                                {e.type === 'fire' ? '🔥' : '🍅'}
                                            </span>
                                        ))}
                                    </div>
                                ...
                                <div style={{ position: 'absolute', bottom: tracks.length > 1 && window.innerWidth < 768 && idx === 1 ? '100px' : '20px', left: '16px', right: '16px', display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', zIndex: 60, pointerEvents: 'none' }}>
